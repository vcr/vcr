  ...
  module Middleware
    ...
    ::RSpec.describe Rack do
      ...
      describe '#call' do
        ...
        it 'calls the provided net_http2 app and returns its response' do
          net_http2_app = double
          expect(net_http2_app).to receive(:call).with(env_hash).and_return(:response)
          instance = described_class.new(net_http2_app) { |c| c.name 'cassette_name' }
          expect(instance.call(env_hash)).to eq(:response)
        end

        it 'uses a cassette when the net_http2 app is called' do
          expect(VCR.current_cassette).to be_nil
          net_http2_app = lambda { |env| expect(VCR.current_cassette).not_to be_nil }
          instance = described_class.new(net_http2_app) { |c| c.name 'cassette_name' }
          instance.call({})
          expect(VCR.current_cassette).to be_nil
        end
        ...
        it 'sets the cassette name based on the provided block' do
          net_http2_app = lambda do |env|
            expect(VCR.current_cassette.name).to eq('net_http2_cassette')
          end
          instance = described_class.new(net_http2_app) { |c| c.name 'net_http2_cassette' }
          instance.call({})
        end
        ...
        it 'sets the cassette options based on the provided block' do
          ...
          instance = described_class.new(net_http2_app, &lambda do |c|
            c.name    'c'
        ...
        it 'yields the net_http2 env to the provided block when the block accepts 2 arguments' do
